...
func TestSignSHA512Encrypt(t *testing.T) {
	crypto := &NativeCrypto{}
	...
	sign, err := crypto.SignSHA512(data, signerKeypair.PrivateKey())
	if err != nil {
		t.Fatal(err)
	}

	if ok, err := crypto.Verify(data, sign, signerKeypair.PublicKey()); err != nil || !ok {
		t.Fatal(err)
	}
